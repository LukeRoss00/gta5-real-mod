...
**If the batch file gives you errors like "The system cannot find the path specified" or "0 file(s) copied":** The `RealConfig.bat` file assumes that your Documents folder is at `%UserProfile%\Documents`. If you moved or redirected it, you should manually edit the batch file in Notepad and replace `%UserProfile%\Documents` with the full path of your Documents folder, then run the batch file again.  
