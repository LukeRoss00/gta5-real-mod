...
**May 27 Update:** *Due to the recent game updates pushed by Rockstar and the different version available on the Epic Games Store, in order to make the R.E.A.L. mod work now you should also download the latest Script Hook V from Alexander Blade's page (http://www.dev-c.com/gtav/scripthookv/) and use it to update the `ScriptHookV.dll` file in the main game folder; the other files in `ScriptHookV_1.0.1868.4.zip` are unchanged and you don't need to replace them.*
